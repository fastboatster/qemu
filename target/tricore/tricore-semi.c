...
/* Various flags defined by newlib.  */
/*OLD*/
...
{
//return value is the amount of char read
...
//  struct ui_file *gdb_stdtargerr;
  if (desc<=2)
    {
      if (desc==0) {
    	  /* rlen=ui_file_read(gdb_stdin,(char *) buf,len); */
      ...
      if (desc==1)  {
    	  /*rlen=ui_file_read(gdb_stdout,(char *) buf,len);*/
      ...
      if (desc==2)  {
    	  /*rlen=ui_file_read(gdb_stderr,(char *) buf,len); */
...
//    for (i=0; i<len; i+=1) fprintf_unfiltered (gdb_stdlog, " %d %x",i,buf[i]);
    if (desc==0) {
    	/*ui_file_write(gdb_stdin,(const char *) buf,len);*/
    ...
    if (desc==1) {
    	/*ui_file_write(gdb_stdout,(const char *) buf,len);*/
    ...
    if (desc==2) {
    	/*ui_file_write(gdb_stderr,(const char *) buf,len);*/
...
//  pc=(uint32_t) uvalue;
//  if (tricore_virtio_addr == (uint32_t) -1)
//    {
